...
`.gitattributes` 文件用于配置文件的属性。文件中的一行对应一个属性。
属性的格式为`pattern attr1 attr2 ...`。

- `text`：控制行尾规范化，即是否将行尾转换为LF
  - `text=auto`：如果是文本文件，则将行尾转换为LF；如果git库中行尾为GRLF，则不转换
- `eol`：控制行尾规范化，检测出工作目录后，是否将行尾转换为LF或CRLF
  > `LF`：Line Feed，换行符`\n`；`CR`：Carriage Return，回车符`\r`；
  - `eol=lf`：将行尾转换为LF
  - `eol=crlf`：将行尾转换为CRLF
- `diff`：控制文件的差异比较
  - `diff=astextplain`：将文件视为文本文件，不进行二进制比较
  - `diff=bin`：将文件视为二进制文件，不进行文本比较
    - `!diff`
