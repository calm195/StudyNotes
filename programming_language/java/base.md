...
29. `@Transient`注解：用于标记不需要序列化的字段
    - 在序列化时，标记为`@Transient`的字段不会被序列化
    - 适用于不需要持久化的字段，如临时变量、缓存等

30. `System.arraycopy()`方法：用于数组的快速复制
    - 声明：`public static native void arraycopy(Object src, int srcPos, Object dest, int destPos, int length)`
    - 参数：
      - `src`：源数组
      - `srcPos`：源数组的起始位置
      - `dest`：目标数组
...
31. `ThreadGroup` 线程组。
    - 声明：`public final class ThreadGroup extends Object implements Serializable`
    - 方法：
      - `ThreadGroup(String name)`：创建一个新的线程组
      - `void setDaemon(boolean daemon)`：设置线程组是否为守护线程组
      - `boolean isDaemon()`：判断线程组是否为守护线程组
      - `int activeCount()`：返回当前活动线程的数量
      - `int activeGroupCount()`：返回当前活动线程组的数量
      - `Thread[] enumerate(Thread[] list)`：将当前线程组中的所有活动线程复制到指定数组中
      - `ThreadGroup[] enumerate(ThreadGroup[] list)`：将当前线程组中的所有活动子线程组复制到指定数组中
...
> SpEL例子：`#{systemProperties['java.version']}`，获取系统属性`java.version`的值。
